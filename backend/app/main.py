...
async def change_password(
    ...
    if not old_password:
        raise HTTPException(status_code=400, detail="oldPassword is required")
    if not new_password:
...
async def get_user_sessions_admin(
    ...
    Returns:
        {
            "sessions": [
                {
                    "id": "...",
                    "module": "payroll area" | "payment method",
                    "updatedAt": "...",
                    "progress": 0-100,
                    "done": true/false,
                    "payrollAreas": [...],  // if module is "payroll area" and done
                    "paymentMethods": [...] // if module is "payment method" and done
                }
            ]
        }
    """
    ...
    sessions_list = get_user_sessions(user_id, None)
    return {
        "sessions": [
            {
                "id": s["id"],
                ...
                "progress": calculate_progress(s["config_state"]),
                "done": s["config_state"].get("done", False),
                "payrollAreas": s["config_state"].get("payroll_areas", []),
                "paymentMethods": s["config_state"].get("payment_methods", []),
            }
            for s in sessions_list
        ]
    }
